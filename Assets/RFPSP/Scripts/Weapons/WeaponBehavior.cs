...
public class WeaponBehavior : MonoBehaviour {
	...
	private TextMeshProUGUI ammoText;//set reference for main color element of ammo GUIText
	...
	void Start (){
		...
		ammoText = FPSPlayerComponent.ammoText.GetComponent<TextMeshProUGUI>();//set reference for main color element of ammo GUIText
	...
	void Update(){
		...
		if(Time.timeScale > 0.0f && Time.deltaTime > 0.0f){//allow pausing by setting timescale to 0
			...
			if(meleeSwingDelay == 0 && !unarmed){
				...
				ammoText.text = $"<size=40>{bulletsLeft}</size>/{ammo}";
			}else{
                ammoText.text = $"<size=40>{bulletsLeft}</size>/{ammo}";
            }
	...
	IEnumerator FireOneShot (){
		...
		for(float i = 0; i < projectileCount; i++){
			...
			if(projectilePoolIndex != 0 && !meleeActive){
				...
				projBody.linearVelocity = Vector3.zero;
	...
	IEnumerator SpawnShell (){
        ...
        ShellEjectionComponent.RigidbodyComponent.linearVelocity = Vector3.zero;

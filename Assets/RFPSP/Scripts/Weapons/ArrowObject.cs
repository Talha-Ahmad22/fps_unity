...
public class ArrowObject : MonoBehaviour {
	...
	public float damageAdd = 20.0f;//how much to increase damage with full strength pull
	...
	public float waitDuration = 30.0f;
	[HideInInspector]
	...
	void Update () {
		...
		if(!hit){
			...
			if(!falling && myRigidbody.linearVelocity.magnitude > 0.01f){
				transform.rotation = Quaternion.LookRotation(myRigidbody.linearVelocity);//make arrow always point in direction of movement
			}
		}else{
	...
	void HitTarget(){	
		...
		&& !arrowRayHit.collider.gameObject.GetComponent<ArrowObject>()){
			...
			transform.gameObject.tag = "Usable";//allow arrow to be picked up
			...
			case 0://hit object
				...
				break;
			case 1://hit object is a breakable object like a window

...
public class ShellEjection : MonoBehaviour{
	...
	public void InitializeShell(){
		...
		RigidbodyComponent.maxAngularVelocity = 1000;//allow shells to spin faster than default

		//determine if shell rotates clockwise or counter-clockwise at random
		if(Random.value < 0.5f){shellRotateUp *= -1;}

        //rotate shell and apply torque to rigidbody
        RigidbodyComponent.AddRelativeTorque(Vector3.up * (Random.Range (0.12f, 0.1f) * shellRotateSide), ForceMode.Impulse);
		RigidbodyComponent.AddRelativeTorque(Vector3.right * (Random.Range (0.25f, 0.15f * 6) * shellRotateUp), ForceMode.Impulse);
		...
		while(true){
			...
			if(!FPSWalkerComponent.playerParented){//if player is not on a moving platform
				...
				|| (FPSWalkerComponent.prone && FPSWalkerComponent.moving))){
					Vector3 tempVelocity = PlayerRigidbodyComponent.linearVelocity;
					tempVelocity.y = 0.0f;
			    ...
			    || (startTime + PlayerWeaponsComponent.shellBtParentTime < Time.time && FPSPlayerComponent.bulletTimeActive)){
					myTransform.parent = null;
					//add player velocity to shell when unparenting from player object to prevent shell from suddenly changing direction
					RigidbodyComponent.AddForce(PlayerRigidbodyComponent.linearVelocity, ForceMode.VelocityChange);	
					parentState = false;
					yield break;
				}		
			}

...
//		llf.GetComponent<LevelLoadFade>().FadeAndLoadLevel(Color.black, 2.0f, true);
	...
	void Update (){
        ...
        if (PlayerStats.Instance.regenEnabled && hitPoints < maximumHitPoints)
        {
            if (Time.time - timeLastDamaged > PlayerStats.Instance.regenDelay)
            {
                hitPoints += PlayerStats.Instance.regenRate * Time.deltaTime;
		...
		healthBar.fillRect.GetComponent<Image>().color = healthColor.Evaluate(hitPoints / 100f);
...
//	}
	...
	public void HealPlayer( float healAmt, bool isHungryThirsty = false ){
		...
		healthText.text = Mathf.Round(hitPoints).ToString();
		healthBar.value = Mathf.Round(hitPoints);

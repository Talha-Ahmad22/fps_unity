...
public class FPSRigidBodyWalker : MonoBehaviour {
	...
	[HideInInspector]
	public PhysicsMaterial playerPhysicMaterial;
...
//		if(Mathf.Abs(InputComponent.moveY) > 0.15f || Mathf.Abs(InputComponent.moveX) > 0.15f){
		if(Mathf.Abs(inputY) > 0.15f || Mathf.Abs(inputX) > 0.15f){
			...
			moving = false;
			if(lowerMoveFriction && !frictionState){
				capsule.material.frictionCombine = PhysicsMaterialCombine.Maximum;
	...
	void FixedUpdate(){
		...
		if(Time.timeScale > 0){//allow pausing by setting timescale to 0
			...
			velocity = RigidbodyComponent.linearVelocity;
			...
			if((climbing || swimming) && !jumping){//climbing var is managed by a ladder script attatched to a trigger that is placed near a ladder
				if(InputComponent.moveY > 0.1f){//only climb if player is moving forward
					...
					RigidbodyComponent.linearVelocity = new Vector3(velocity.x, verticalSpeedAmt, velocity.z);
					
				}else{
					//if not moving forward, do not add extra upward velocity, but allow the player to move off the ladder
					RigidbodyComponent.linearVelocity = new Vector3(velocity.x, 0, velocity.z);
				}
			}
			...
			if(RigidbodyComponent.linearVelocity.y > verticalSpeedLimit){
				Vector3 clampVel = RigidbodyComponent.linearVelocity;
				clampVel.y = Mathf.Clamp(clampVel.y, -500.0f, verticalSpeedLimit);
				RigidbodyComponent.linearVelocity = clampVel;
			}

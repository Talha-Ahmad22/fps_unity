...
public class CharacterDamage : MonoBehaviour {
    ...
    [Header("Ammo Drop Prefabs (Match order with weapon selection)")]
    public GameObject[] ammoDropPrefabs; // Size 2: [0] = Weapon A, [1] = Weapon B

    [Tooltip("Chance (0 to 1) to drop ammo on death.")]
    [Range(0f, 1f)]
    public float ammoDropChance = 0.3f;

    [Tooltip("Chance (0 to 1) to drop coins on death.")]
    [Range(0f, 1f)]
    public float coinDropChance = 0.5f;
    public int coinGainAmount = 1; // Amount of coins to give per kill
...
//						}
	...
	public void ApplyDamage ( float damage, Vector3 attackDir, Vector3 attackerPos, Transform attacker, bool isPlayer, bool isExplosion, Rigidbody hitBody = null, float bodyForce = 0.0f ){
		...
		damage = PlayerStats.Instance.damageMultiplier * damage;
		if (hitPoints <= 0.0f){
			return;
		}
		...
		if (hitPoints <= 0.0f){
			...
			if(bodies.Length < 2){//if NPC is only using one capsule collider for collision, instantiate ragdoll, instead of activating existing body part rigidbodies
				...
				if(!ragdollActive){
					WaveManager.instance.AddKill(1);
    ...
    void TryDropLoot()
    {
		int randomVal = Random.Range(0, 2); //To make sure the enemies don't always drop loot
        ...
        {
            int index = GameManager.selectedWeaponIndex;

            if (index >= 0 && index < ammoDropPrefabs.Length && ammoDropPrefabs[index] != null)
            {
                Instantiate(ammoDropPrefabs[index], transform.position + Vector3.up * 0.5f, Quaternion.identity);
            }
        }

        // Coin gain (no physical drop)
        if (Random.value < PlayerStats.Instance.bonusCoinChance)
        {
            GameManager.Instance.AddCoins(PlayerStats.Instance.bonusCoinAmount);
            // Optionally show visual feedback like floating text
        }
